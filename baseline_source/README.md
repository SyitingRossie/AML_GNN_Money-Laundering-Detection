...
(1)原demo是先整理节点标签和节点特征后，再进行随机切分训练集和验证集，对节点打掩码。因此，训练集和验证集都用了全局的节点特征，例如：特征有每个账户的各种货币的平均收（付）款金额。如果训练集就统计了测试集在内的平均收付款金额，那就产生了泄露，容易出现验证集指标虚高，模型在真实线下业务场景推理时效果大幅滑坡，泛化能力差。
