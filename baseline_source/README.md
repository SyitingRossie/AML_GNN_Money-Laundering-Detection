...
# 三、原基线代码存在的核心缺陷
1.严重的数据泄露问题：原作先将所有账户特征一并整理后，再进行随机切分，切90%训练集和10%验证集，因此账户节点特征已经包含了10%的测试集数据特征，例如特征列有每个账户的不同货币进款（或出款）的平均金额。如果训练集就统计了测试集在内的平均收付款金额，容易出现验证集指标虚高，模型在真实线下业务场景推理时效果大幅滑坡，泛化能力差。
