...
3.1.1 原demo是先整理节点标签和节点特征后，再进行随机切分训练集和验证集，对节点打掩码。因此，训练集和验证集都用了全局的节点特征，例如：特征有每个账户的各种货币的平均收（付）款金额。如果训练集就统计了测试集在内的平均收付款金额，那就产生了泄露，容易出现验证集指标虚高，模型在真实线下业务场景推理时效果大幅滑坡，泛化能力差。

3.1.2 训练集和验证集的标签都是全局的。例如，只要数据集里该节点有过洗钱交易标签，那么该节点标签始终是1。反之，正常客户也是如此。那么训练集就偷看了也许验证集的是否洗钱的标签信息。

3.1.3 由于交易边用的是一张全局的图，训练集图卷积也卷了验证集的交易边信息，模型偷看了训练集信息。
...
(3)、训练集标签窗口：70%-85%交易获取。
(4)、验证集标签窗口：85%-100%交易获取。
(5)、验证集节点，评估的是0-70%里的节点。
(6)、验证集的节点特征和交易边，均来自0-70%训练集的数据。
