...
基线痛点：原demo是基于所有交易统计节点标签和节点特征后，再进行随机切分训练集和验证集，但没有考虑到金融行业实际应用中的时序问题。

(1)、训练集和验证集均采用全局的节点特征，训练集的节点已经融合了验证集的交易数据，产生了严重的数据泄露。容易出现验证集指标虚高，模型在真实线下业务场景推理时效果大幅滑坡，泛化能力差。

(2)、训练集和验证集的标签都是全局标签。例如：a节点全局有过洗钱交易标签，那么该节点标签始终是1。反之，正常客户也是如此，导致训练阶段提前知道了未来的洗钱结果。

(3)、由于交易边用的是一张全局的图，训练集卷积了未来时序生成的交易边，致使训练节点利用了未来资金流向的网络结构。

优化方案：采用时序窗口切分（交易按时间排序后再切分）

（1）、训练图基于0-70%交易构建，预测70-85%交易时段的账户洗钱标签；验证图基于0-85%交易构建，预测85-100%交易时段的账户标签。
（2）、训练、验证阶段独立构图，独立统计节点特征，交易边特征，交易标签。
（3）、训练集的主节点及其标签是既在70-85%之间的节点又在0-70%之间的节点，交易边是在0-70%之间所有的与主节点相关的节点均参与训练。同理，验证集类似。验证集主节点及标签是在85-100%之间的节点且在0-85%之间的节点，交易边是0-85%图里的所有交易边信息。

训练窗口按交易数据的时间排序，然后先切分前70%作为训练集的图来源。
